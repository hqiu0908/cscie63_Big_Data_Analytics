...
public class CQLClient {
	...
	public void loadData() {
		session.execute(
				"INSERT INTO newkeyspace.person (id, fname, lname, city, phone1, phone2, phone3) " +
						"VALUES (" +
	...
	public void querySchema(){
		...
		System.out.println(String.format("%-33s\t%-10s\t%-10s\t%-10s\t%-15s\t%-15s\t%-15s\n%s", 
				"id", "lname", "fname", "city", "phone1", "phone2", "phone3", 
				"---------------------------------------+---------------+---------------+---------------+---------------+---------------+---------------"));
		for (Row row : results) {
			System.out.println(String.format("%-5s\t%-10s\t%-10s\t%-10s\t%-15s\t%-15s\t%-15s", row.getUUID("id").toString(),
